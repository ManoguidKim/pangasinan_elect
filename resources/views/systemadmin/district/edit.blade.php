<x-app-layout>
    ...
    <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-2">Update district details</h3>
    <form action="{{ route('admin-manage-district-edit', $district->id) }}" method="post">
        @csrf
        <div class="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div class="w-full">

                <label class="block mb-1 text-xs font-medium text-gray-900 dark:text-white italic">District</label>
                <input type="text" name="district" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 mb-2" placeholder="Enter here" value="{{ $district->name }}">
                @error('district')
