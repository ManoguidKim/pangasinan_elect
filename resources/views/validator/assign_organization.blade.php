<x-app-layout>
    ...
    <h2 class="mb-4 text-3xl font-bold leading-none tracking-tight text-gray-500 md:text-3xl dark:text-white">Assingning Organization to {{ $voter->fname . ' ' . $voter->mname . ' ' . $voter->lname  }}</h2>
    <form action="{{ route('system-admin-validator-voter-assign-organization-save', $voter->id) }}" method="post">
